class ClassStatusesController < ApplicationController
    ...
    respond_to do |format|
      if ClassStatus.applied?(user_id, class_id)
		...
		format.html { redirect_to root_path, notice: '수강인원을 초과할 수 없습니다.' }
      	...
      	format.html { redirect_to root_path, notice: '18 학점을 초과할 수 없습니다.' }
    
	  else
        @class_status.save
        format.json { redirect_to root_path, notice: '강의 신청이 완료되었습니다.' }
      end
    end
  end
  ...
  def destroy
  	...
  	respond_to do |format|
    	format.html { redirect_to class_statuses_url, notice: '강의 신청 취소, 학점이 차감되었습니다.' }
